...
for components (host page must also load Lucide and call `lucide.createIcons()`).
...
ESP32-S3 — there is no DOM, no CSS, no Lucide. Use these files as the source of truth for colors,
...
- The physical control set (Ø36 push-select dial + 4× Ø13 caps: play/pause, skip fwd, skip back,
  change rooms) is what `core/board.h` has to expose for this board.

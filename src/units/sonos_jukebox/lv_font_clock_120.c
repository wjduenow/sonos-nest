/*******************************************************************************
 * Montserrat-Medium, 120 px, 4 bpp — DIGITS, COLON, HYPHEN AND SPACE ONLY.
 *
 * WHY THIS FILE EXISTS. The screensaver clock needs to be readable from across a room, and LVGL
 * ships Montserrat only up to 48 px. The alternative — scaling a 48 px label with
 * `transform_scale` — allocates a full ARGB8888 draw layer from the LVGL pool on every redraw:
 * "12:34" at 2.5x is roughly 240 KB out of a 512 KB pool, and pool exhaustion on this stack is a
 * UI FREEZE (LVGL answers a failed layer alloc with a retry loop), not a dropped frame. That is
 * the same trap CLAUDE.md records against the nest's scaled clock. A real font costs flash — which
 * this board has ~4 MB of spare — instead of the one resource that hangs it.
 *
 * The range is deliberately tiny: 0-9 and ':' for the time, '-' for the "--:--" shown before NTP
 * lands, and space. AM/PM is drawn beside it in montserrat_28, so no letters are needed here.
 * Widening the range is the expensive direction — every glyph at 120 px is ~3 KB.
 *
 * Generated, not hand-authored — regenerate rather than edit. The TTF is LVGL's own, so this
 * matches every other face in the UI exactly:
 *   cp .pio/libdeps/sonos-jukebox/lvgl/scripts/built_in_font/Montserrat-Medium.ttf .
 *   npx lv_font_conv --font Montserrat-Medium.ttf --size 120 --bpp 4 \
 *       --range '0x20,0x2D,0x30-0x3A' --format lvgl --lv-include lvgl.h --no-compress \
 *       -o lv_font_clock_120.c
 *
 * --no-compress is required: LVGL only decodes compressed fonts with LV_USE_FONT_COMPRESSED, which
 * this project does not enable. Montserrat is SIL OFL 1.1 (see lvgl/scripts/built_in_font/
 * font_license). Compiled only into UNIT_JUKEBOX — it is in units/sonos_jukebox/.
 ******************************************************************************/
...
#ifndef LV_FONT_CLOCK_120
#define LV_FONT_CLOCK_120 1
#endif

#if LV_FONT_CLOCK_120
...
static const lv_font_fmt_txt_glyph_dsc_t glyph_dsc[] = {
    ...
    {.bitmap_index = 27037, .adv_w = 436, .box_w = 17, .box_h = 65, .ofs_x = 5, .ofs_y = -1}
};
...
#if LVGL_VERSION_MAJOR >= 8
const lv_font_t lv_font_clock_120 = {
#else
lv_font_t lv_font_clock_120 = {
#endif
